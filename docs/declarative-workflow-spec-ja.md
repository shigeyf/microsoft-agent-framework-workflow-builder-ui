...
- 到達不能なアクションには「⚠ 到達不能」バッジを付け、カードを淡く表示する
- 実行を終了する 5 種（`EndWorkflow` / `EndDialog` / `EndConversation` / `CancelDialog` / `CancelAllDialogs`）は、最上位にある場合のみ Output へ点線で結ぶ

ジャンプ系の 3 種（`GotoAction` / `BreakLoop` / `ContinueLoop`）は制御が別の場所へ移るだけでワークフローを終わらせないため、Output へは結ばない。
